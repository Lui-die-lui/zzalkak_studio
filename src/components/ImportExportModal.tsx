...
export function ImportExportModal({ editor }: ImportExportModalProps) {
  ...
  const handleImportFile = async (file: File) => {
    ...
    const { merged, imported, renamed } = mergeImportedTemplates(templates, validation.templates);
  ...
  return (
    <Modal open={editor.ioModalOpen} title="템플릿 가져오기 / 내보내기" onClose={close}>
      <section className="io-section">
        <h3 className="io-section__title"><Download size={16} /> JSON 내보내기</h3>
        <p className="io-section__desc">저장된 템플릿 전체({templates.length}개)를 JSON 파일 하나로 내려받습니다. 이미지 파일은 포함되지 않습니다.</p>
        <button type="button" className="btn btn--secondary" onClick={handleExport} disabled={templates.length === 0}>
      ...
      <section className="io-section">
        <h3 className="io-section__title"><Upload size={16} /> JSON 가져오기</h3>
        <p className="io-section__desc">
          내보낸 JSON 파일을 다시 불러옵니다. 파일 전체를 먼저 검증한 뒤에만 반영하므로, 문법이 깨졌거나 필수 항목이 빠진 파일은 아무것도 바꾸지 않고 거부됩니다.
